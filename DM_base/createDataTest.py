...
filePath1 = 'Test/pred'
# filePath2 = 'Test/Combined/masks2m'
filePath3 = 'Test/dm'
...
def readImagesTwice(files1, name1, name3):
    L = len(files1)
    X = []
    Y = []
    # Z = []
    ...
    for f1 in files1:
        ...
        # mask = misc.imread(name2 + "/" + f1.replace("tif", "png"), mode= 'L').astype('float32')
        ...
        #     img = img / img.max()
        dm = dm / 255.
	    # org = misc.imread(name4 + "/" + f1).astype('float32')
        # if org.max():
        #     # print img.max(), dm.max(), mask.max()
        #         org = org / org.max()
        # mask = (mask - 0.5) / 0.5
...
[X, Y] = readImagesTwice(fileList1, filePath1, filePath3)
np.save('dmMT.npy', Y)
np.save('albuTstMT.npy', X)
# np.save('segTrnR.npy', Z)
