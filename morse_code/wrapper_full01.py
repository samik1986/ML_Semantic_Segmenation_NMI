    ...
    for f1 in sorted(files1):
        if f1 not in files2:
                ...
                ############################# Channel Red ##############################################################
                            ...
                            _, thresh1 = cv2.threshold(np.uint8(out_img),0.2*255,255,cv2.THRESH_BINARY)
                            # print(thresh1.nonzero())
                            maskN[thresh1.nonzero()] = 11
                            count = count + 1
                ############################ Channel Green ######################################################
                            ###################### ALBU ###################
                            tileN = np.zeros((512,512,3)).astype(np.uint8)
                            ...
                            _, thresh2 = cv2.threshold(np.uint8(out_img),0.2*255,255,cv2.THRESH_BINARY)
                            # print(thresh2.nonzero())
                            maskN[thresh2.nonzero()] = maskN[thresh2.nonzero()]+12
                            maskN = np.clip(maskN,0,13)
                            count = count + 1
                            #################### Stitching ###############
                            op[row:row+512, col:col+512] = maskN
                # pred_time = time.time()
        #################### write back Mask ###############
                ...
                cv2.imwrite(outDir + f1, np.uint8(op1))
