...
eps = 0.0001
filePath = '../wdata/test/final_test/images/'
dmPath = '../wdata/test/final_test/raw/'
...
# print fileList1
def testImages(files1, inDir, dmPath, outDir):
    L = len(files1)
    count = 0
    for f1 in sorted(files1):
        ...
        # tile = img
        tile = cv2.imread(dmPath + "/" + f1.replace('_img.', '.'), cv2.IMREAD_UNCHANGED)
        # print(tile.max())
        dm_op = dm_fn(tile,count)
        # dm_op = dm_op / 255.
        ...
        # print(w,h)
        # tile = img
        albu_op = albu_dingkang.predict(models_albu, img, image_type='8_bit_RGB')
        print(albu_op.max(), albu_op.min())
        # albu_op = albu_op / 255.
...
## Dingkang: Here I load all four models of albu. Need to put fold i_best.pth files under folder albu_weights.
...
#     run()
testImages(fileList1, filePath, dmPath, outDir)
