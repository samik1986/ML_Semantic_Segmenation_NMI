...
model = dmnet('dmnet_membrane_4Dec.hdf5')
...
fileList1 = os.listdir('newpred/')
fileList2 = os.listdir('newdm/')
...
def testImages(files1, name1, name2):
    ...
    for f1 in files1:
        ...
        dm = misc.imread(name2 + "/" + f1)
        dm = dm.astype('float32')
        img = img / 255.
        dm = dm / 255.
        print(img.min(), dm.min())       # if img.max():
        #     img = img / img.max()
        # dm = dm / 255.
...
#        print model.summary()
        ...
        misc.imsave("results_MBA_new/" + f1, img.astype('uint8'))
        

testImages(fileList1,'newpred/', 'newdm/')
