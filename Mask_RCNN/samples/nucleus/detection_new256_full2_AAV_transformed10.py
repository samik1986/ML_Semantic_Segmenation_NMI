...
def image2mask(image, maskB, opR, opG,  w, h, flagR=False, flagG=False):
    window = 512
    count = 0
    for row in range(0, w - window, window):
        for col in range(0, h - window, window):
            ...
            if np.sum(tileM): 
                ...
                tileR = np.zeros((window, window, 3), dtype= np.uint8)
                tileR[:, :, 0] = tile[:, :, 0]
                ...
                cv2.imwrite('outN/R_' + str(count) + '.tif', np.uint8(outK)*255)
                outK[row + window - 10:row + window, col + window - 10:col + window] = False
                outK[row :row + 10, col :col + 10] = False
                outK[row :row + 10, col + window - 10:col + window] = False
                outK[row + window - 10:row + window, col :col + 10] = False
                opR[row:row + window, col:col + window] = outK
                # op[row:row + window, col:col + window] = outK*10
            
                tileG = np.zeros((window, window, 3), dtype=np.uint8)
                tileG[:, :, 1] = tile[:, :, 1]
                ...
                outK = np.asarray(outK[1])
                outK = binary_fill_holes(outK)
                cv2.imwrite('outN/G_' + str(count) + '.tif', np.uint8(outK)*255)
                outK[row + window - 10:row + window, col + window - 10:col + window] = False
                outK[row :row + 10, col :col + 10] = False
                outK[row :row + 10, col + window - 10:col + window] = False
                outK[row + window - 10:row + window, col :col + 10] = False
                opG[row:row + window, col:col + window] = outK
                # op[row:row + window, col:col + window] = op[row:row + window, col:col + window] + outK*20
                count = count + 1   
    return opR, opG

weights_path = 'PMD_model_Cell_RG.h5'
...
for files in fileList1:
    ...
    opR, opG = image2mask(image, maskB, opR, opG, w, h)
    op = np.uint8(opR) * 10 + np.uint8(opG) * 20
