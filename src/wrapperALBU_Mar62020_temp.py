...
eps = 0.0001
brainNo = 'PMD2055'
filePath = '/nfs/data/main/M32/Process_Detection/ProcessDetPass1/temp/'
...
def testImages(files1, files2, inDir, outDir):
    ...
    for f1 in sorted(files1):
        if f1 not in files2:
                ...
                opR = np.zeros((w,h), dtype=np.uint8)
                opG = np.zeros((w,h), dtype=np.uint8)
                ################## Tiling #####################
                id = []
                tile = []
                tile = []
                ...
                ############################# Channel Red ##############################################################
                            ...
                            # _, thresh1 = cv2.threshold(albu_op1,0.3*255,255,cv2.THRESH_BINARY)
                            # # print(thresh1.nonzero())
                            # maskN[thresh1.nonzero()] = 255
                            # maskRGB[:,:,1] = maskN
                            # maskRGB[:,:,2] = maskN
                            opR[row:row+512, col:col+512] = albu_op1
                            count = count + 1
                ############################ Channel Green ######################################################
                            ###################### ALBU ###################
                            maskN = np.zeros((512,512),dtype='uint8')
                            ...
                            # _, thresh2 = cv2.threshold(albu_op2,0.3*255,255,cv2.THRESH_BINARY)
                            ...
                            count = count + 1
                            #################### Stitching ###############
                            opG[row:row+512, col:col+512] = albu_op2
                # pred_time = time.time()
        #################### write back Mask ###############
                ...
                cv2.imwrite(outDir + f1[:-4] + '.maskR.jp2', np.uint8(opR))
                cv2.imwrite(outDir + f1[:-4] + '.maskG.jp2', np.uint8(opG))
